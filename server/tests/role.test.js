...
describe("Role Routes", () => {
  ...
  it("POST /create-role → should create a new role", async () => {
    const res = await request(app).post("/api/create-role").send({
      name: "Admin",
  ...
  it("GET /roles → should get all roles", async () => {
    await request(app).post("/api/create-role").send({
      name: "Manager",
  ...
  it("PUT /update-role/:id → should update role", async () => {
    const res = await request(app).put(`/api/update-role/${roleId}`).send({
  ...
  it("DELETE /delete-role/:id → should delete role", async () => {
    ...
    const res = await request(app).delete(`/api/delete-role/${tempRoleId}`);
  ...
  it("GET /role/:id → should return null for non-existent role", async () => {
    ...
    const res = await request(app).get(`/api/role/${fakeId}`);
